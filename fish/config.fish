...
if [ $TERM = 'linux' ]
    ...
    setfont /usr/share/kbd/consolefonts/ter-g28b.psf.gz
end

# ===== abbrs =====
abbr -a  s-pac   sudo pacman
abbr -a  s-pacs  sudo pacman -S
abbr -a  s-pacsy sudo pacman -Sy
abbr -a  s-pacr  sudo pacman -Rsn
abbr -a  s-cp    sudo -E cp
abbr -a  s-mv    sudo -E mv
abbr -a  s-v     sudo -E nvim

abbr -a  s-  sudo -E
abbr -a  +x  sudo chmod +x
...
# ===== env vars =====
# --- java ---
set -x JAVA_HOME "/opt/oracle/jdk1.8.0_211"
...
set -x EDITOR nvim
# --- fzf config ---
#set -x FZF_PREVIEW_COMMAND '[[ $(file --mime {}) =~ binary ]] && echo {} is a binary file || cat -n {} | head -500'
set -x FZF_PREVIEW_COMMAND 'head -c 1024 {} | cat -ns'
set -x FZF_DEFAULT_OPTS "--bind ctrl-j:down,ctrl-k:up --border --color=dark --tabstop=4 --preview '$FZF_PREVIEW_COMMAND'"
#set -x FZF_DEFAULT_COMMAND 'find \( -path "**/.git" -o -path "**/links" -o -path "**/.*" -o -empty \) -a -prune -o -type f'
set -x FZF_DEFAULT_COMMAND 'find \( -path "**/.git" -o -path "**/links" \) -a -prune -o -type f'
